...
class TemperatureRuntime:
    def __init__(self,config):
        ...
        self.config=config;self.wafer='';self.batch=0;self.completed=[];self.current={};self.cache={};self.errors=[]

    def on_event(self,kind,payload):
        if kind=='lot_start':self.tp.reset_bias()
        elif kind=='wafer_start':
            self.wafer=payload['wafer'];self.batch=0;self.completed=[];self.current={};self.cache={};self.errors=[];self.tp.end_all()
        elif kind=='test_start':
            self.batch+=1;self.tp.end_all();self.cache={};self.current={}
            ...
            self.current={};self.tp.end_all();self.cache={}
    ...
    def predict(self,k):
        ...
        for site in self.current:
            if self.tp.target_key[k] in self.tp.buf.get(site,{}):
                raise ValueError('Prediction requested after target measurement; refusing hindsight prediction')
        values={}
        for site,d in self.current.items():
            v,info=self.tp.predict_site(k,site);values[site]=round(v,3)
    ...
    def snapshot(self):
        return {'wafer':self.wafer,'batch':self.batch,'devices':copy.deepcopy(self.completed+list(self.current.values())),
