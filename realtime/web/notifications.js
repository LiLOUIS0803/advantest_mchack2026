...
const render=window.renderVisuals;window.renderVisuals=(s,id)=>{render(s,id);confidence.textContent=s.classification_confidence?`Confidence ${score(s.classification_confidence)} · Uncalibrated${s.classification_confidence.partial_data ? " | Partial data" : ""}`:'';};
